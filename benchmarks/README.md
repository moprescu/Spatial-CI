...
* The final results are generated by the notebook [`benchmarks/leaderboard.ipynb`](https://github.com/moprescu/Spatial-CI/blob/main/benchmarks/leaderboard.ipynb). This notebook must be run after running the full experiments.
* Hyperparameter tuning plots are generated through: [`benchmarks/tune_plots.ipynb`](https://github.com/moprescu/Spatial-CI/blob/main/benchmarks/tune_plots.ipynb)



## Reproducibility (from the original SPACE README.md)

**These instructions may be useful if you want to run on a cluster, or you may want to look at the `nersc` branch to see the changes made to run on a cluster.**
