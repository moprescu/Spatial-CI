...
class CVAE(pl.LightningModule):
    ...
    def on_save_checkpoint(self, checkpoint):
        # assumes you track val_p_value in validation_epoch_end
        checkpoint["val_p_value"] = float(self.current_val_p_value)
...
class CVAEDataset(Dataset):
    def __init__(self, dataset, nodes, coords2id, radius, treat_scaler=None, feat_scaler=None, output_scaler=None, a=None, change=None, datatype="grid"):
...
class UNetDataset(Dataset):
    def __init__(self, dataset, nodes, coords2id, radius, treat_scaler=None, feat_scaler=None, output_scaler=None, a=None, change=None, datatype="grid", latent=None):
        ...
        self.covariates = torch.cat([self.covariates, torch.from_numpy(latent)], dim=-1)
...
class Deconfounder(SpaceAlgo):
    ...
    def fit(self, dataset: SpaceDataset):
        ...
        train_ix, test_ix, _ = spatial_train_test_split_radius(
            ...
            init_frac= 0.02, 
            levels = 1, 
            buffer = 1 + max(dataset.radius, self.cvae_kwargs["radius"]), 
            radius = max(dataset.radius, self.cvae_kwargs["radius"]),
        )
        ...
        nbrs = {node: get_k_hop_neighbors(graph, node,  max(dataset.radius, self.cvae_kwargs["radius"])) for node in node_list}
        ...
        self.cvae_traindata = CVAEDataset(dataset, self.train_ix, self.max_coords2id, self.cvae_kwargs["radius"], datatype=dataset.datatype)
        self.cvae_valdata = CVAEDataset(dataset, self.test_ix, self.max_coords2id, self.cvae_kwargs["radius"], self.cvae_traindata.treat_scaler, self.cvae_traindata.feat_scaler, self.cvae_traindata.output_scaler, datatype=dataset.datatype)
        ...
        if self.head == "spatialplus" or self.head == "s2sls-lag1":
            ...
            spatialplusdata = CVAEDataset(dataset, self.max_nodes, self.max_coords2id, self.cvae_kwargs["radius"], datatype=dataset.datatype)
        ...
        if self.head == "unet":
            ...
            self.head_traindata = UNetDataset(dataset, self.train_ix, self.max_coords2id, self.cvae_kwargs["radius"], datatype=dataset.datatype, latent=train_latents)
            self.head_valdata = UNetDataset(dataset, self.test_ix, self.max_coords2id, self.cvae_kwargs["radius"], self.head_traindata.treat_scaler, self.head_traindata.feat_scaler, self.head_traindata.output_scaler, datatype=dataset.datatype, latent=val_latents)
    ...
    def tune_metric(self, dataset: SpaceDataset) -> float:
        ...
        if self.head == "unet":
            ...
            predict_data = CVAEDataset(dataset, self.max_nodes, self.max_coords2id, self.cvae_kwargs["radius"], self.cvae_traindata.treat_scaler, self.cvae_traindata.feat_scaler, self.cvae_traindata.output_scaler, datatype=dataset.datatype)
    ...
    def predict(self, dataset: SpaceDataset, nodes, a, change) -> dict:
        ...
        predict_data = CVAEDataset(dataset, nodes, self.max_coords2id, self.cvae_kwargs["radius"], self.cvae_traindata.treat_scaler, self.cvae_traindata.feat_scaler, self.cvae_traindata.output_scaler, datatype=dataset.datatype, a=a, change=change)
        ...
        if self.head == "unet":
            predict_head_data = UNetDataset(dataset, nodes, self.max_coords2id, self.cvae_kwargs["radius"], self.head_traindata.treat_scaler, self.head_traindata.feat_scaler, self.head_traindata.output_scaler, a=a, change=change, datatype=dataset.datatype, latent=latents)
