...
class GMLag(SpaceAlgo):
    ...
    def predict(self, dataset: SpaceDataset):
        ...
        rho = float(self.model.rho[0])
        beta = self.model.betas.flatten()
        W = self.model.w.sparse  # scipy sparse matrix

        # Compute fitted values: (I - rho W)^(-1) X beta
        n = W.shape[0]
        I = np.eye(n)
        y_hat = np.linalg.inv(I - rho * W.toarray()) @ (x @ beta)
