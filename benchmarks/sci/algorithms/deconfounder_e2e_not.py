...
class CVAE_Grid(nn.Module):
    ...
    def forward(self, treatments, covariates):
        ...
        return treatment_probs, mu, logvar, z_s
...
class E2E_CVAE(pl.LightningModule):
    def __init__(
        self,
        feature_dim: int,
        radius: float,
    ...
    def predict_step(self, batch, batch_idx):
        ...
        treatment_probs, mu, logvar, z_s = self.model(treatments, covariates)
        treat = treatments.permute(0, 3, 1, 2)
        feat = covariates.permute(0, 3, 1, 2)
        z = z_s.view(-1, self.model.patch_size, self.model.patch_size, self.model.per_location_latent_dim).permute(0, 3, 1, 2)
        head_input = torch.cat([treat, feat,  z], dim=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        treatment_loss, kldiv_loss, outcome_loss = self.loss(treatment_probs, mu, logvar, out, true_treatments, true_outcomes)
        # Combined objective
...
class Deconfounder_E2E(SpaceAlgo):
    """
    ...
    def fit(self, dataset: SpaceDataset):
        ...
        self.unet_kwargs["n_channels"] = 1 + dataset.covariates.shape[1] + (self.cvae_kwargs["encoder_conv2"] // 2)
        self.unet_kwargs["radius"] = max(self.cvae_radius, dataset.conf_radius)
        
        self.head = UNet(**self.unet_kwargs)
        self.cvae_model = E2E_CVAE(**self.cvae_kwargs, head=self.head)
    ...
    def predict(self, dataset: SpaceDataset, nodes, a, change) -> dict:
        ...
        preds = torch.cat(self.cvae_trainer.predict(self.cvae_model, loader))
        preds = preds.cpu().numpy()
        preds = self.cvae_traindata.output_scaler.inverse_transform(preds)
