...
def main(cfg: DictConfig):
    ...
    if datatype == "graph":
        ...
        # ====== EXP 2: CONFOUNDER-MODULATED SPILLOVER ======
        # Modify the outcome BEFORE fitting so the DGP function f learns
        # the confounder-modulated interference term:
        #     Y_new = Y + α · U_s · Ā_Ns
        # where U_s = q_summer (masked confounder), Ā_Ns = mean neighbor
        # treatment, and α is calibrated so that the added term has
        # std ≈ 0.5 · std(Y).
        ...
        U_s_exp2 = df_grid[U_col_exp2].values.astype(np.float64)
        U_s_exp2 = np.nan_to_num(U_s_exp2, nan=0.0)

        # Mean neighbor treatment Ā_Ns from grid features
        treat_nbr_pos_exp2 = [
            (dr, dc) for dr in range(-radius, radius + 1)
                     for dc in range(-radius, radius + 1)
                     if not (dr == 0 and dc == 0)
        ]
        treat_nbr_grid_cols_exp2 = [
            f"{spaceenv.treatment}_{dr}_{dc}" for dr, dc in treat_nbr_pos_exp2
        ]
        A_Ns_exp2 = df_grid[treat_nbr_grid_cols_exp2].mean(axis=1).values
        A_Ns_exp2 = np.nan_to_num(A_Ns_exp2, nan=0.0)

        # Calibrate α so std(α · U · Ā_Ns) ≈ 0.5 · std(Y_original)
        Y_orig = df_grid[spaceenv.outcome].values.astype(np.float64)
        Y_orig_finite = Y_orig[np.isfinite(Y_orig)]
        raw_term_exp2 = U_s_exp2 * A_Ns_exp2
        raw_std_exp2 = float(np.std(raw_term_exp2))
        target_std_exp2 = 0.5 * float(np.nanstd(Y_orig_finite))
        if raw_std_exp2 > 1e-10:
            alpha_exp2 = target_std_exp2 / raw_std_exp2
        else:
            alpha_exp2 = 0.0
        logging.info(
            f"EXP 2: α = {alpha_exp2:.4g}  "
            f"(target std {target_std_exp2:.4g}, raw std {raw_std_exp2:.4g})"
        )

        # Inject modified outcome — predictor will learn the new DGP
        df_grid[spaceenv.outcome] = Y_orig + alpha_exp2 * raw_term_exp2
        # Keep df in sync (used by counterfactual generation paths)
        df.loc[df_grid.index, spaceenv.outcome] = df_grid[spaceenv.outcome]
        # ===================================================
        ...
        # === Skip feature importance and leave-one-out loop ===
        # For these experiment variants we don't need feature importance
        # or per-covariate confounding scores; leave them empty.
        logging.info("Skipping feature importance computation (variant DGP).")
        featimp = {c: 0.0 for c in covariates}
        featimp["treatment"] = 0.0
        treat_featimp = {c: 0.0 for c in covariates}
        cs_minimum = {c: 0.0 for c in covariates}
        top_feat = {}
        top_featcols = []
        cs_erf = {}
        cs_ite = {}
