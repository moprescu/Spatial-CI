...
def main(cfg: DictConfig):
    ...
    for a in avals:
        ...
        predicted = treat_predictor.predict(cfdata)
    ...
    for i, g in enumerate(covar_groups):
        ...
        cols = dftrain.columns.difference(extra_colnames + value_ + [spaceenv.outcome])
        
        leave_out_predictor = TabularPredictor(label=spaceenv.treatment)
        ...
        cf_err = (leave_out_mu_cf.values - mu_cf.values) / tscale
        cs_ite[key_] = float(np.sqrt((cf_err**2).mean(0)).mean())
        logging.info(f"ITE: {cs_ite[key_]:.3f}")

        erf_err = (leave_out_mu_cf.values - mu_cf.values).mean(0) / tscale
